...
%    \item $x, x' \in X'$: Sensitive tokens; $y \in Y$: Output of the token sanitization mechanism. Following previous work, we consider each token as being represented by a real vector $\mathbb{R}^\ell$ for some constant $\ell$. Also, similar to other works, we consider $Y=X'$.
    ...
    %\item Mapping $f'$ between vectors defined based on embedding $f$ and cluster centroids is called a {\em cluster embedding}.
...
\setlength\itemsep{0em}
    ...
    \item \textbf{Inputs}: Parameter $k$, token embedding $f$ which maps each token to a real vector, i.e., $f:X'\cup Y\rightarrow \real^\ell$, a distance $d_c:\real^\ell \times \real^\ell \rightarrow \real$, a clustering $\{C\}$ of $X' \cup Y$
...
\begin{enumerate}
    ...
    % \begin{itemize}
    %     \item 
        Run  exponential mechanism $M_E(x)$ parametrised by $\epsilon_E = \epsilon/2$, where the input space is $X'$ (all tokens) with the token embedding $f$, the output space is $C$ (tokens in cluster $C$), utility $u$ is $u(x, x') = -d(x, x')$, and $\Delta u = \max_{x, x', y\in X'} |d(x, y) - d(x', y)|$
    % \end{itemize}
    \item Output the token chosen above.
\end{enumerate}
    }%
